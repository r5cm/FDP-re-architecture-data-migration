...
map.v2v1 <- data.frame(gs_read(template.info, "V2", "A1:I365"))
      ...
      for(j in 1:nrow(map.temp)) {
            if(map.temp$type_v2[j] != "reference") {
                  query.temp <- paste("SELECT", map.temp$field_v1[j], "FROM",
                                      fdp.objects.1[i],
                                      "WHERE test_mig2__c = 2 ORDER BY Id", sep = " ")
...
# save data
saveRDS(data.2, "data_2_1_raw.RDS")
...
# Retrieve saved data
data.2 <- readRDS("data_2_1_raw.RDS")

# Change all Diagnostic/monitoring types to Diagnostic
...
for(i in seq_along(check.rep)) {
      ...
      if(sum(temp.dup) > 1) {
            print(paste("DUPLICATED records in ", check.rep[i], ": ",
                        paste(data.2[[i]]$farmer.code[temp.dup], collapse = ", "),
                        sep = ""))
      } else {
...
saveRDS(data.2, "data_2_2_clean.RDS")

# 4. Transform and load -----------------------------------------------------------

# Retrieve data to load
data.2 <- readRDS("data_2_2_clean.RDS")


# FAMILY MEMBERS (TRANSFORM)

# Select visited farmers
farmer.visit <- data.2$fdp_farmer__c[!is.na(data.2$fdp_farmer__c$agree.fdp), ]
farmer.visit <- farmer.visit[farmer.visit$agree.fdp == "Ya", ]
farmer.visit <- farmer.visit[!is.na(farmer.visit$Family_members__c),]

# Spouse
# Create family members data frame for all spouses
family.members <- select(farmer.visit, Id_v1__c, spouseName__c, gender__c,
...
table(farmer.visit$gender__c, useNA = 'always')
h.gender <- data.frame(incorrect = c("Male", "Female"), correct = c("Pria", "Wanita"))
for(i in 1:nrow(farmer.visit)) {
      pos <- grep(farmer.visit$gender__c[i], h.gender$incorrect)
      if(length(pos) != 0 & !is.na(farmer.visit$gender__c[i])) {
            farmer.visit$gender__c[i] <- h.gender$correct[pos]
      }
}
farmer.visit$gender__c <- as.character(farmer.visit$gender__c)
farmer.visit$gender__c[farmer.visit$gender__c == "N/A"] <- NA
...
# farmers.null <- rep(NA, nrow(farmer.visit))
# farmer.fm <- with(farmer.visit,
...
table(farmer.visit$have_spouse_indic, useNA = 'always')
# # Check that all family members are not empty
# table(farmer.visit$Family_members__c, useNA = 'always')
# farmer.visit$farmerCode__c[is.na(farmer.visit$Family_members__c)]
# Check that all with spouse should have at least two family members
test.fm <- with(farmer.visit,
                have_spouse_indic == "1.0" & as.numeric(as.character(Family_members__c)) < 2)
sum(test.fm, na.rm = TRUE) #Should be 0
farmer.visit$Family_members__c <- as.character(farmer.visit$Family_members__c)
farmer.visit$Family_members__c[test.fm] <- "2"
# Replace all 0 family members with 1 (the farmer)
fm.0 <- farmer.visit$Family_members__c == "0.0"
farmer.visit$Family_members__c[fm.0] <- "1.0"
farmer.visit$Family_members__c <- as.factor(farmer.visit$Family_members__c)
...
sum(as.numeric(as.character(farmer.visit$Family_members__c))) == nrow(family.members)
...
saveRDS(data.2, "data_2_3_famMembers.RDS")

data.2 <- readRDS("data_2_3_famMembers.RDS")
...
rm(temp.children)
# Organization: all to Mars (UPDATE FOR PRODUCTION INSTANCE: UPDATED)
data.2$fdp_farmer__c$organization__c <- "a1R0K000004QQSJ"
...
batches_info <- rforcecom.createBulkBatch(session.2, 
                                          jobId=job_info$id, 
                                          select(data.2$fdp_farmer__c, -agree.fdp),
                                          multiBatch = TRUE, 
                                          batchSize=500)
...
# Status
data.2$fpd_Submission__c$Status__c <- gsub("Ya", "Agreed", 
                                           data.2$fpd_Submission__c$Status__c)

# farmer
id.farmer.2 <- rforcecom.retrieve(session.2, "fdp_farmer__c", 
                                  c("Id", "Id_v1__c"))

# _________________________________________
# Only for creating missing batch
# _________________________________________
id.farmer.2 <- rforcecom.retrieve(session.2, "fdp_farmer__c", 
                                  c("Id", "Id_v1__c", "CreatedDate"))
id.farmer.2 <- id.farmer.2[grepl("2018-07-11", id.farmer.2$CreatedDate), ]
id.farmer.2 <- select(id.farmer.2, -CreatedDate)
# _________________________________________
# Back to old script
# _________________________________________
...
rm(temp.sub)
# surveyor: SKIP THIS PART WHEN IN PRODUCTION INSTANCE
...
# _________________________________________
id.subm.2 <- rforcecom.retrieve(session.2, "fpd_Submission__c", 
                                c("Id", "Id_v1__c", "Respondent__c", "CreatedDate"))
id.subm.2 <- id.subm.2[grepl("2018-07-11", id.subm.2$CreatedDate), ]
id.subm.2 <- select(id.subm.2, -CreatedDate)
# _________________________________________
# Back to old script
# _________________________________________
...
temp.fmsub <- left_join(data.2$fdp_Family_members__c, id.subm.2, 
                        by = c("FDP_submission_v1__c" = "Id.subm.1"))
...
# Only for creating missing batch
# _________________________________________
id.farm.2 <- rforcecom.retrieve(session.2, "fdp_Farm__c", 
                                c("Id", "Id_v1__c", "farmer__c", "CreatedDate"))
id.farm.2 <- id.farm.2[grepl("2018-07-11", id.farm.2$CreatedDate), ]
id.farm.2 <- select(id.farm.2, -CreatedDate)
# _________________________________________
# Back to old script
# _________________________________________
...
# Only for creating missing batch
# _________________________________________
id.farmbl.2 <- rforcecom.retrieve(session.2, "fdp_Farm_BL__c", 
                                  c("Id", "Id_v1__c", "CreatedDate"))
id.farmbl.2 <- id.farmbl.2[grepl("2018-07-11", id.farmbl.2$CreatedDate), ]
id.farmbl.2 <- select(id.farmbl.2, -CreatedDate)
# _________________________________________
# Back to old script
# _________________________________________
...
# Only for creating missing batch
# _________________________________________
id.plot.2 <- rforcecom.retrieve(session.2, "fdp_plot__c",
                                c("Id", "Id_v1__c", "CreatedDate"))
id.plot.2 <- id.plot.2[grepl("2018-07-11", id.plot.2$CreatedDate), ]
id.plot.2 <- select(id.plot.2, -CreatedDate)
# _________________________________________
# Back to old script
# _________________________________________
...
id.diag.mon <- rforcecom.retrieve(session.2, "fdp_Diagnostic_Monitoring__c",
                                  c("Id", "Id_v1__c", "CreatedDate"))
id.diag.mon <- id.diag.mon[grepl("2018-07-12", id.diag.mon$CreatedDate), ]
id.diag.mon <- select(id.diag.mon, -CreatedDate)
# _________________________________________
# Back to old script
# _________________________________________
...
# Only for creating missing batch
# _________________________________________
names(data.2$fpd_observation__c)[5] <- "Response__c"
# _________________________________________
# Back to old script
# _________________________________________
...
saveRDS(data.2, "Data_2_4_loadedFarmer.RDS")


# USER PERFORMANCE

data.2 <- readRDS("Data_2_4_loadedFarmer.RDS")
...
# Submission (REVISAR POR QU? HAY SUBMISSIONS DUPLICADOS)
...
data_loaded <- readRDS("Data_2_4_loadedFarmer.RDS")
data_raw <- readRDS("data_2_1_raw.RDS")

# Submission repeated (loaded)
# Farmer
subm_loaded <- data.frame(table(data_loaded$fdp_farmer__c$FDP_submission_v1__c))
subm_loaded_rep <- subm_loaded[subm_loaded$Freq > 1, ]
# Farm
subm_loaded <- data.frame(table(data_loaded$fdp_Farm__c$FDP_submission_v1__c))
subm_loaded_rep <- subm_loaded[subm_loaded$Freq > 1, ]
# Farm baseline
subm_loaded <- data.frame(table(data_loaded$fdp_Farm_BL__c$FDP_submission_v1__c))
subm_loaded_rep <- subm_loaded[subm_loaded$Freq > 1, ]

# Submission repeated (raw)
subm_raw <- data.frame(table(data_raw$fdp_farmer__c$FDP_submission_v1__c))
subm_raw_rep <- subm_raw[subm_raw$Freq > 1, ]
...
batches_info <- rforcecom.createBulkBatch(session.2, 
                                          jobId=job_info$id, 
                                          select(data.2$fpd_observation__c, -Result__c),
                                          multiBatch = TRUE,
                                          batchSize=500)
# check on status of each batch
batches_status <- lapply(batches_info, 
...
>>>>>>> ddb781c2f0d26405b53cf9442aab611c6d57894f
